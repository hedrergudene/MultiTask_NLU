...
def main(
    ...
    train_dts = NER_Dataset(data.loc[train_idx,'utt'].values, train_intent, train_dct['HuggingFace_model'], max_length, nlp, intent2idx, ner2idx)
    val_dts = NER_Dataset(data.loc[val_idx,'utt'].values, val_intent, train_dct['HuggingFace_model'], max_length, nlp, intent2idx, ner2idx)
    # Define model
    print(f"Get model:")
